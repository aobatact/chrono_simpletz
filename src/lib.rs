...
impl<const HOUR : i32, const MINUTE : u32> UtcZst<HOUR,MINUTE>{
    const OFFSET_SECS : i32 = HOUR * HOUR_TO_SEC + if HOUR < 0 {-1} else {1} * (MINUTE as i32) * MIN_TO_SEC;
}
impl<const HOUR : i32, const MINUTE : u32> Offset for UtcZst<HOUR,MINUTE>{
    fn fix(&self) -> FixedOffset { FixedOffset::east(Self::OFFSET_SECS)}
}
...
mod tests {
    ...
    fn it_works() {
        ...
        assert_eq!(n.naive_utc(),NaiveDate::from_ymd(2000, 1, 1).and_hms(3, 0, 0));
        let m9 = UtcM9::default();
        let n = m9.ymd(2000,1,1).and_hms(12,00,00);
        assert_eq!(n.naive_utc(),NaiveDate::from_ymd(2000, 1, 1).and_hms(21, 0, 0));
        let p9 = UtcP9_30::default();
        let n = p9.ymd(2000,1,1).and_hms(12,00,00);
        assert_eq!(n.naive_utc(),NaiveDate::from_ymd(2000, 1, 1).and_hms(2, 30, 0));
        let m9 = UtcM9_30::default();
        let n = m9.ymd(2000,1,1).and_hms(12,00,00);
        assert_eq!(n.naive_utc(),NaiveDate::from_ymd(2000, 1, 1).and_hms(21, 30, 0));
    }
